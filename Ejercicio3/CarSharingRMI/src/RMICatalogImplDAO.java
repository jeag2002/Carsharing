...
public class RMICatalogImplDAO implements RMICatalogInterfaceDAO{
	...
	@Override
	public RMICarTO showCar(int car) throws Exception {
		
		PreparedStatement pstmt = null;
		Connection conn = null;
		RMICarTO carVO = new RMICarTO();

		try {
			System.out.println("findCar called");

			//Get DB connection
			conn = getConnection();

			pstmt = conn.prepareStatement("SELECT * FROM postgres.practicaepcsd.car WHERE id = ?");
			pstmt.setInt(1, car);
			ResultSet rs = pstmt.executeQuery();
			...
			while (rs.next()) {
				...
				carVO.setModel(rs.getString("model"));				
				carVO.setBrand(rs.getString("brand"));
				carVO.setPhoto(rs.getString("photo"));
				carVO.setColor(rs.getString("color"));
				carVO.setNif(rs.getString("nif"));		
			}
	...
	public Connection getConnection() throws Exception {
		try {			
	        ...
	        conn = DriverManager.getConnection("jdbc:postgresql://localhost:5432/postgres","USER","homer23");			
			return conn;
		}
		catch (Exception e) {
